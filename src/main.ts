...
const ball: Ball = new Ball('pink', x, y);
const paddle: Paddle = new Paddle(paddleX, canvas.height - 10);
const trackScore: GameLabel = new GameLabel(8, 20, 'Score: ', score);
const trackLives: GameLabel = new GameLabel(canvas.width - 65, 20, 'Lives: ', lives);
const background: Background = new Background(0, 0, canvas.width, canvas.height);
const gameBricks: Bricks = new Bricks();
